...
func taskgetPublicUserData (_ completionHandlerForUserData: @escaping (_ success: Bool, _ errorString: String?) -> Void) {
    ...
    let task = Client.taskForGETMethods(urlString: urlString) { (results, error) in
        ...
        if let error = error {
            ...
            if let last_name = results?[parametersAll.StudentLocation.publicfirstName] as? String,
                ...
                // This user object can be returned back using closure
...
//            if let last_name = results?[parametersAll.StudentLocation.publiclastName] as? NSDictionary {
//                if let first_name = results?[parametersAll.StudentLocation.publiclastName] as? NSDictionary{
